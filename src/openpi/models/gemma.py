...
def get_config(variant: Variant) -> Config:
    ...
    if variant == "gemma_llm_small_220m":
        return Config(
            width=512,
            depth=18,
            mlp_dim=2048,
    ...
    if variant == "gemma_llm_small_120m":
        # smaller sibling of 220M (~120M params)
        return Config(
            width=384,
            depth=18,
            mlp_dim=1536,
    ...
    # very small versions
    if variant == "gemma_tiny_8m":
        # ~5–10M params (very rough)
        # Shared constraints you can also use on the action expert.
        return Config(
            width=96,
            depth=6,
            mlp_dim=384,       # 4x width
            num_heads=4,       # keep small but >=2
            num_kv_heads=1,
            head_dim=32,       # attention inner = num_heads * head_dim (not required to equal width here)
        )

    if variant == "gemma_nano_3m":
        # ~2–4M params (very rough)
        return Config(
            width=64,
            depth=6,
            mlp_dim=256,       # 4x width
            num_heads=4,
            num_kv_heads=1,
            head_dim=32,
        )
        
    if variant == "gemma_micro_800k":
        # ~0.5–0.9M params (per expert), depending on exact wiring
        return Config(
            width=64,
            depth=3,
            mlp_dim=256,     # 4× width
            num_heads=4,
            num_kv_heads=1,
            head_dim=32,     # heads*head_dim = 128
        )

    if variant == "gemma_pico_300k":
        # ~0.25–0.35M params (per expert)
        return Config(
            width=48,
            depth=3,
            mlp_dim=192,     # 4× width
