...
class Args:
    ...
    eval_task_idx: int = 0  # Task id to evaluate
...
def eval_libero_sequence(args: Args) -> None:
    """Evaluate a single task and compute lifelong-learning metrics."""
    ...
    assert 0 <= args.eval_task_idx < n_total, (
        f"eval_task_idx must be < {n_total}, got {args.eval_task_idx}")
    ...
    row = args.eval_task_idx
    max_steps = _get_max_steps(args.task_suite_name)
    client = _websocket_client_policy.WebsocketClientPolicy(args.host, args.port)

    def _count_task_trajectories(video_dir: str, task_description: str) -> int:
        task_seg = task_description.replace(" ", "_")
        vdir = pathlib.Path(video_dir)
        if not vdir.exists():
            return 0
        pattern = str(vdir / f"rollout_{task_seg}_*.mp4")
        return len(glob.glob(pattern))

    total_episodes, total_successes = 0, 0
    for task_id in tqdm.tqdm(range(row, row + 1), desc="Tasks"):
        task = task_suite.get_task(task_id)
        initial_states = task_suite.get_task_init_states(task_id)
        env, task_description = _get_libero_env(task, LIBERO_ENV_RESOLUTION, args.seed)

        existing = _count_task_trajectories(video_dir, task_description)
        if existing > 49:
            logging.info(
                f"Skipping task '{task_description}' — found {existing} trajectories in '{args.video_out_path}'"
            )
            continue
