#!/usr/bin/env python3
"""
eval_continual_per_task.py

For a given checkpoint step, starts the policy server, evaluates
all tasks up to the given task index, then shuts the server down.
"""
...
def run_eval(task_idx, ckpt_dir, port):
    ...
    env = os.environ.copy()
    cmd = [
        "python", "examples/libero/main_single.py",
        "--args.task_suite_name", TASK_SUITE,
